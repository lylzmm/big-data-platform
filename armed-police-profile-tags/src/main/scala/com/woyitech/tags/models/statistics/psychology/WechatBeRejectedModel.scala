 ...
 * 计算周期：  每个用户近一年内数据
 * 更新周期：  1天
 * 字段模型：  wechat_be_rejected_number
 *
 */

object WechatBeRejectedModel {

  def main(args: Array[String]): Unit = {

    val sql =
      """
        |select cast(user_id as string) user_id,  cast(ct as string)  wechat_be_rejected_number
        |from (
        ...
        |                  where chat_content like '%对方已拒绝%' and t1.chat_audit_id = t2.chat_audit_id and substr(t1.chat_time, 1, 10) >= date_sub(t2.data_date, 365)
        |              ) a
