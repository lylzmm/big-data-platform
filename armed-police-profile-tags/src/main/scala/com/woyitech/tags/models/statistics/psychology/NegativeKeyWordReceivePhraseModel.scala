 ...
 * 计算周期：  每个用户近一年内数据
 * 更新周期：  1天
 * 字段模型：  receive_negative_phrase
 *
 */

object NegativeKeyWordReceivePhraseModel {

  def main(args: Array[String]): Unit = {

    val sql =
      s"""
         |select cast(user_id as string) user_id, concat_ws('|', collect_set(concat(keyword, '-', ct))) receive_negative_phrase
         |from (
