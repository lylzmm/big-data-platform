 ...
 * 计算周期：  每个用户近一年内数据
 * 更新周期：  1天
 * 字段模型：  refused_to_phone_number
 * 字段说明：  msg_type  1:通话,2:短信   |    call_type  通话：1被叫, 2:主叫'
 */

object PhoneRefusedToModel {

  def main(args: Array[String]): Unit = {

    val sql =
      """
        |select cast(user_id as string) user_id, cast(sum(ct) as string) refused_to_phone_number
        |from (
        ...
        |                           where  msg_type = 1  and call_Duration = '0' and call_type = 1 and t1.user_id = t2.user_id and substr(t1.call_time,1 ,10) >= date_sub(t2.data_date, 365)
        |                       ) b
