 ...
 * 计算周期：  每个用户近一年内数据
 * 更新周期：  1天
 * 字段模型：  wechat_phone_time
 */

object WeChatPhoneTimeModel {

  def main(args: Array[String]): Unit = {
    val sql =
      """
        |select cast(user_id as string) user_id,cast(wechat_phone_time as string) wechat_phone_time
        |from (
        |         select chat_audit_id,round(sum(points), 2)  wechat_phone_time
        |         from (
        |                  -- 求出每一条语音通话的分钟
        ...
        |                                                      where chat_content like '%语音通话:聊天时长%' and substr(chat_time, 1, 10) < substr(current_date, 1, 10)
        |                                                  ) a
        |                                         ) b
        |                                    where rp <= 365
        |                                ) c
