 ...
 * 计算周期：  每个用户近一年内数据
 * 更新周期：  1天
 * 字段模型：  wechat_receive_negative_phrase
 */

object WeChatNegativeKeyWordReceivePhraseModel {

  def main(args: Array[String]): Unit = {

    val sql =
      """
        |select cast(user_id as string) user_id, concat_ws('|', collect_set(concat(keyword, '-', ct))) wechat_receive_negative_phrase
        |from (
