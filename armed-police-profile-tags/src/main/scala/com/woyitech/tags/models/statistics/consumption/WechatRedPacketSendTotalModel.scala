 ...
 * 计算周期：  最近一个月
 * 更新周期：  1天
 * 字段模型：  wechat_red_packet_send_total
 * 字段说明：  content_type  352表示转账 272表示红包   |    chat_status  收到：0  发出：1
 */
object WechatRedPacketSendTotalModel {

  def main(args: Array[String]): Unit = {

    val sql =
      """
        |select cast(user_id as string) user_id, cast(money as string) wechat_red_packet_send_total
        |from (
        ...
        |         -- chat_status 1 发
        |         where content_type = 272 and chat_status = 1 and t1.chat_audit_id = t2.chat_audit_id and substr(t1.chat_time, 1, 10) >= date_sub(t2.data_date, 30)
