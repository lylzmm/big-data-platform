...
import org.apache.hadoop.hbase.client.{ConnectionFactory, Put, Result, ResultScanner, Scan, Table}
...
object HBaseTools {
  ...
  def read(spark: SparkSession, zks: String, port: String, table: String, family: String, fields: Seq[String]): DataFrame = {
    ...
    val rowSchema: StructType = StructType(
      fields.map { field =>
        StructField(field, StringType, nullable =
          true)
      }
    )
    // 7. 转换为DataFrame
  ...
  def write(dataframe: DataFrame, zks: String, port: String, table: String, family: String, rowKeyColumn: String): Unit = {
    if (rowKeyColumn != null) {
      ...
      // 4. 保存数据到表
      putsRDD.coalesce(1).saveAsNewAPIHadoopFile(
