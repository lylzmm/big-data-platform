...
public class CarLoans {
    ...
    public static String carLoans(String line) {
        ...
        if (line.contains("元")) {
            if ( line.contains("金额")) result = getMoney(line, "金额");
            if (result.length() == 0 && line.contains("应还")) result = getMoney(line, "应还");
            if (result.length() == 0 && line.contains("还款")) result = getMoney(line, "还款");
            if (result.length() == 0 && (line.contains("月供"))) result = getMoney(line, "月供");
        }
        // 验证金额是否正常，
        if (3 <= result.split("\\.")[0].length() && result.split("\\.")[0].length() <= 5 && result.contains(".")) {
            String msgOriginName = StringUtil.matchingSource(line);
            return msgOriginName + "->8->" + result;
        } else {
            // 有异常的金额打印出来
    ...
    public static void main(String[] args) throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("input/车贷.csv")));
        String line = null;
        while ((line = br.readLine()) != null) {
            String borrowing = carLoans(line);
            if (borrowing.contains("->8->")) {
